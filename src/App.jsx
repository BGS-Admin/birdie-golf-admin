  ...
  /* ═══════════════════════════════════════════
    ...
    content=<div style={S.pad}>
      ...
      {memModal&&<div style={S.ov} onClick={()=>setMemModal(null)}><div style={S.mod} onClick={e=>e.stopPropagation()}>
        ...
        <label style={GS.label}>CUSTOMER</label>
        <div style={{display:"flex",gap:6,marginBottom:10}}>
          <button style={{...GS.togBtn,...(!memModal.newCust?{background:GREEN,color:"#fff",borderColor:GREEN}:{})}} onClick={()=>setMemModal(p=>({...p,newCust:false,cust:null,firstName:"",lastName:"",phone:"",email:""}))}>Existing</button>
          <button style={{...GS.togBtn,...(memModal.newCust?{background:GREEN,color:"#fff",borderColor:GREEN}:{})}} onClick={()=>setMemModal(p=>({...p,newCust:true,cust:null}))}>New Customer</button>
        </div>

        {memModal.newCust?<div style={{background:"#fafaf8",borderRadius:10,padding:12,display:"flex",flexDirection:"column",gap:8,marginBottom:12}}>
          <div style={{display:"flex",gap:8}}><div style={{flex:1}}><label style={{...GS.label,fontSize:10}}>FIRST NAME *</label><input style={GS.input} value={memModal.firstName||""} onChange={e=>setMemModal(p=>({...p,firstName:e.target.value}))}/></div><div style={{flex:1}}><label style={{...GS.label,fontSize:10}}>LAST NAME *</label><input style={GS.input} value={memModal.lastName||""} onChange={e=>setMemModal(p=>({...p,lastName:e.target.value}))}/></div></div>
          <div><label style={{...GS.label,fontSize:10}}>PHONE *</label><input style={GS.input} value={memModal.phone||""} onChange={e=>setMemModal(p=>({...p,phone:e.target.value}))}/></div>
          <div><label style={{...GS.label,fontSize:10}}>EMAIL</label><input style={GS.input} value={memModal.email||""} onChange={e=>setMemModal(p=>({...p,email:e.target.value}))}/></div>
        </div>:<>
          <input style={{...GS.input,marginBottom:4}} placeholder="Search by name or phone..." value={custSearch} onChange={e=>setCustSearch(e.target.value)}/>
          {custSearch&&<div style={{border:"1px solid #e8e8e6",borderRadius:8,marginTop:0,maxHeight:150,overflowY:"auto",marginBottom:12}}>
            {customers.filter(c=>(!c.tier||c.tier==="none")&&(cn(c).toLowerCase().includes(custSearch.toLowerCase())||(c.phone||"").includes(custSearch))).slice(0,8).map(c=>
              <div key={c.id} style={{padding:"8px 12px",borderBottom:"1px solid #f2f2f0",cursor:"pointer",fontSize:13}} onClick={()=>{setMemModal(p=>({...p,cust:c}));setCustSearch("");}}><span style={{fontWeight:600}}>{cn(c)}</span> <span style={{color:"#888",fontSize:11}}>{c.phone}</span></div>)}
          </div>}
          {memModal.cust&&<p style={{fontSize:13,fontWeight:600,color:GREEN,marginBottom:12}}>✓ {cn(memModal.cust)}</p>}
        </>}
        ...
        {(memModal.cust||(memModal.newCust&&memModal.firstName&&memModal.phone))&&<>
          ...
          <input type="date" style={{...GS.input,marginBottom:14}} min={dateKey(new Date(Date.now()+86400000))} value={memModal.renewalDate||""} onChange={e=>setMemModal(p=>({...p,renewalDate:e.target.value}))}/>

          <button style={{...S.b1,opacity:(memModal.tier&&memModal.renewalDate)?1:0.35}} onClick={async()=>{
            if(!memModal.tier||!memModal.renewalDate)return;
            const t=TIERS.find(x=>x.id===memModal.tier);
            let custId=memModal.cust?.id;
            if(memModal.newCust){
              const nc=await db.post("customers",{phone:(memModal.phone||"").replace(/[^0-9]/g,""),first_name:memModal.firstName,last_name:memModal.lastName||"",email:memModal.email||"",tier:"none"});
              if(nc?.[0])custId=nc[0].id;
              await sq("customer.create",{first_name:memModal.firstName,last_name:memModal.lastName,phone:(memModal.phone||"").replace(/[^0-9]/g,""),email:memModal.email});
            }
            if(!custId){fire("Could not create customer");return;}
            ...
            await db.post("membership_history",{customer_id:custId,action:"join",tier:memModal.tier,amount:t.p});
            ...
            const dispName=memModal.newCust?memModal.firstName+" "+memModal.lastName:cn(memModal.cust);
            fire(dispName+" added as "+t.n+" ✓");setMemModal(null);setCustSearch("");load();
          }}>Add Member</button>
        </>}
