...
import { CSS, S, TEAM, X, ff, mono, GREEN } from "./lib/constants.jsx";
...
export default function AdminApp() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [c, b, bl, pr] = await Promise.all([
  ...
  if (!logged) return (
    <div style={LS.w}>
      ...
      <div style={LS.c}>
        ...
        {TEAM.map(t => (
          <button key={t.id} style={LS.rb} onClick={() => { setUN(t.name); setLogged(true); }}>
    ...
    <div style={S.shell}>
      ...
      <div style={S.side}>
        ...
        <div style={{ padding: "12px 16px", borderTop: "1px solid #1a3d2a" }}>
          <button style={{ background: "none", border: "none", color: "#ffffff66", fontSize: 11, cursor: "pointer", fontFamily: ff, display: "flex", alignItems: "center", gap: 6 }} onClick={() => setLogged(false)}>
            {X.out(14)} Sign Out
          </button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        <div style={{ flex: 1, overflowY: "auto" }}>
          {view === "res"      && <ReservationsTab customers={customers} bookings={bookings} bayBlocks={bayBlocks} cfg={cfg} fire={fire} reload={load} />}
          {view === "cust"     && <CustomersTab customers={customers} bookings={bookings} onRefresh={load} />}
          {view === "members"  && <MembersTab customers={customers} fire={fire} reload={load} />}
          {view === "reports"  && <ReportsTab bookings={bookings} customers={customers} />}
          {view === "facility" && <FacilityTab bayBlocks={bayBlocks} setBayBlocks={setBayBlocks} cfg={cfg} setCfg={setCfg} fire={fire} reload={load} />}
        </div>
      </div>
